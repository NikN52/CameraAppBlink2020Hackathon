...
public class ManyLights_Led_Matrix {
...
//                frame = get_rotate(frame.clone(), 270);
...
//        Imgproc.resize(input_image.clone(), printimg, sz);
        Imgproc.rectangle(printimg, new Point(x0, y0), new Point(x1, y1),new Scalar(255, 0, 0, 255), 2);
...
//        int number_of_base_points = 3;
        int total_number_of_points_to_search = 110;
        Mat gray = new Mat();
...
//        HighGui.waitKey(2);
        ...
        double pix_to_mm_ratio =  (double) Lmax / (double) 120;
        ...
        sector = prepare_sector(pix_to_mm_ratio, xL, yL, 62-3, -43, 5, 5);
        s1 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 62-3, -33, 5, 5));
        s2 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 62-3, -22, 5, 5));
        s3 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 62-3, -11, 5, 5));
        s4 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 62-3, -2, 5, 5));
        s5 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 63-4, 8, 5, 5));
        s6 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 63-5, 19, 5, 5));
        s7 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 64-5, 30-2, 5, 5));
        s8 = get_image_independent_sector(thresh, sector);


        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 64-5, 41-3, 5, 5));
        s9 = get_image_independent_sector(thresh, sector);


        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 64-5, 52-3, 5, 5));
        s10 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 64-5, 72-4, 5, 5));
        d2 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 43-5, 72-4, 5, 5));
        d1 = get_image_independent_sector(thresh, sector);

        sector = new HashMap<>(prepare_sector(pix_to_mm_ratio, xL, yL, 86-8, 72-4, 5, 5));
...
//                    System.out.println("Совпадение бля");
    ...
    static ArrayList<ArrayList<Double>> mainly(ArrayList<Mat> video, int start, int finish, Scalar s1, Scalar s2, int rad, int eq_bright_level, int th_MinVal, int th_MaxVal, int alpha, int beta) {
        /*
        ...
        */
        ...
        System.out.println("Декодирование информации из кадров...");
    ...
    static Scalar find_scalars_2(ArrayList<Mat> video, int rad, int eq_bright_level, int alpha, int beta, Scalar s1, Scalar s2)
    {
        System.out.println("Автоопределение HSV порогов..");

        double pos_sd_level = 0.9;
        Mat big = new Mat();
        Mat hsv = new Mat();
        ...
        Scalar sc2 = s2;
        for (int i=60; i<150; i+=10) //value
        {
            for (int j = 40; j < 100; j += 10) //saturation
            {

                System.out.println("Яркость: "+i+"; Насыщенность: "+j);
                Scalar sc1 = new Scalar((int) s1.val[0], j, i);
                ...
                for (int k = 0; k < video.size(); k += 4)
                {
                    ...
                    if (xr_list.size()>1 && yr_list.size()>1)
                    {
                        ...
                        if (k>video.size()/5 && temp_summ<pos_sd_level && temp_summ_yr<pos_sd_level && xR>5 && yR > 5)
                            /*0.6, 0.45*/
                        {
...
//                            System.out.println(Math.abs(yR-yL));
                        ...
                        else if (k>video.size()/5) {k = video.size();}

                    }
                }
                System.out.println("Отклонение X: "+temp_summ);
                System.out.println("Отклонение Y: "+temp_summ_yr);
            }
        }
